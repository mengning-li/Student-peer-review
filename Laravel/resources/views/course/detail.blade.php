                ...
                <table class="table table-striped table-hover">
                    <thead class="thead-dark">
                        <tr>
                            ...
                            <th scope="col">Most Useful Reviews</th>
                            <th scope="col"></th>
                        </tr>
                    </thead>
                    <tbody class="table-group-divider">
                        @if ($assessments)
                            ...
                            <tr>
                                <td><a href="{{ route('assessment.detail', ['course_id' => $course->id, 'assessment_id' => $assessment->id]) }}">
                                        {{ $assessment->title }}
                                    </a>
                                </td>
                                <td>{{ $assessment->due_date }}</td>
                                
                                <td><a href="{{ route('review.useful', ['course_id' => $course->id, 'assessment_id' => $assessment->id]) }}">
                                        Useful reviews
                                    </a>
                                </td>
                                ...
                                <td><button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                                    <a href="{{route('assessment.edit' ,[$course->id, $assessment->id])}}">Edit</a>                     
                                </button></td>
                                @endif
                            </tr>
                            @endforeach
                        @else
                            <tr>
                                <td class="text-muted" colspan="2">No assessment found.</td>
                            </tr>
                        @endif                            
                    </tbody>
                </table>    
